...
The following steps will lead you through this process.

#### Step 1 (uninstall anaconda, if necessary)

Open a terminal. Check if anaconda is installed by running the following command:

```zsh
which anaconda-navigator
```

If you see anything other than `anaconda-navigator not found`, then anaconda is installed. In this case, we very strongly recommend that you do the following two things before proceeding:

* [Uninstall Navigator](https://docs.anaconda.com/navigator/uninstall/)
* [Uninstall Anaconda Distribution](https://docs.anaconda.com/anaconda/install/uninstall/) (including "conda initialization scripts")
...
If this process succeeds, then conda is installed, and you should skip to the next step (**do not** install conda again). If this process does not succeed, then you still need to install conda, and you should continue with this step.

Follow the instructions for [Installing on macOS](https://docs.conda.io/projects/conda/en/latest/user-guide/install/macos.html). It is easiest to use the [Miniforge installer](https://github.com/conda-forge/miniforge/), which requires only running the following commands in a terminal:
...
Please check very carefully (by reading the output in the terminal) that all five of these commands ran to completion without producing any errors. If you see any errors, you need to go back and fix them now, before doing anything else.
...
As an alternative to VS Code, if you like, you can also work with jupyter notebooks in a browser window. To do so, change your working directory to `ae353-sp25` and run this command:
