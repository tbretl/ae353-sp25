...
class Simulator:
    ...
    def __init__(
        ...
        # Set moment of inertia of wheel about its axis of rotation
    ...
    def set_inertia(self, inertia):
        self.inertia = inertia
        # FIXME: You really should change (1) the mass and (2) the other two diagonal
        #        elements of the MOI matrix to match the choice of "inertia" (i.e., the
        #        moi about the axis of rotation). The reason you should do this is to
        #        avoid non-physical moments of inertia - remember that there are inequality
        #        constraints on the three diagonal elements. I am not sure if pybullet
        #        checks these constraints. Do this at some point in future.
        self.bullet_client.changeDynamics(self.wheel_id, 1, localInertiaDiagonal=[0.5, 0.5, self.inertia])
    ...
    def meshcat_init(self):
        ...
        # Add target
        color = self._convert_color([252 / 255, 179 / 255, 22 / 255, 1.])
        self.vis['target'].set_object(
    ...
    def meshcat_update(self):
        ...
        # Set pose of target
        x = self.target_radius * np.cos(self.target_angle)
        y = self.target_radius * np.sin(self.target_angle)
        z = self.target_height
        self.vis['target'].set_transform(
            meshcat.transformations.translation_matrix([
                x * np.cos(self.roll) + z * np.sin(self.roll), y, - x * np.sin(self.roll) + z * np.cos(self.roll),
            ]),
        )
    ...
    def run(
        ...
        while True:
            all_done = self.step(controller, target)
    ...
    def step(self, controller, target):
        # Never stop early
        all_done = False
        ...
        # Change target angle if necessary
        self.target_angle = target(self.t)
        
        # Get the sensor measurements
        wheel_angle, wheel_velocity = self.get_sensor_measurements()

        # Get the wheel torque command (run the controller)
        wheel_torque_command = controller.run(self.t, wheel_angle, wheel_velocity, self.target_angle)
        ...
        self.data['target_angle'].append(self.target_angle)
        for key in self.variables_to_log:
